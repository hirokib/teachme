...
import { getExploration, type SpaceDetail } from './exploration-api';
...
export function ExplorationSpacePage() {
  ...
  return <div className="mx-auto max-w-4xl"><Link to="/explore" className="text-sm text-muted-foreground hover:underline">← Explorations</Link><h1 className="my-6 text-3xl font-semibold">{detail.space.title}</h1>{roots.length ? render(null) : <p>No threads yet.</p>}</div>;
}
