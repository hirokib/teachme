        ...
        {messages.map((message) => {
          ...
          return <article key={message.id} className="group max-w-[90%] rounded-2xl bg-muted/60 px-4 py-3 text-sm" onMouseUp={(event) => captureSelection(visibleMessage, event.currentTarget)}>
            ...
            {verification && <div className="mt-4 rounded-xl border bg-background p-4" onMouseUp={(event) => event.stopPropagation()}>
              ...
              {verification.status === 'completed' && verification.result && <div className="space-y-4"><div><div className="flex items-center justify-between gap-3"><p className="font-semibold">Verification</p><span className="rounded-full bg-accent px-2.5 py-1 text-xs font-medium text-accent-foreground">{VERIFICATION_LABEL[verification.result.overallStatus]}</span></div><Prose className="mt-2 text-sm text-muted-foreground">{verification.result.summary}</Prose></div>{verification.result.claims.map((claim, index) => <div key={`${claim.claim}-${index}`} className="border-t pt-3"><div className="flex items-start justify-between gap-3"><MathText className="font-medium">{claim.claim}</MathText><span className={`shrink-0 rounded-full px-2 py-1 text-[11px] font-medium ${VERDICT_STYLE[claim.verdict]}`}>{claim.verdict.replace('_', ' ')}</span></div><Prose className="mt-1 text-xs text-muted-foreground">{claim.explanation}</Prose>{claim.sources.length > 0 && <div className="mt-2 flex flex-wrap gap-2">{claim.sources.map((source) => <a key={source.url} href={source.url} target="_blank" rel="noreferrer" className="rounded-lg border px-2.5 py-1.5 text-xs font-medium text-primary hover:bg-accent">{source.title} ↗</a>)}</div>}</div>)}</div>}
            </div>}
