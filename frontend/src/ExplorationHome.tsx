...
export function ExplorationHome() {
  ...
  return <div className="mx-auto max-w-5xl space-y-10">
    ...
    <form onSubmit={create} className="pop flex gap-3 rounded-2xl bg-card p-6"><input autoFocus required value={title} onChange={(event) => setTitle(event.target.value)} placeholder="What are you exploring?" className="flex-1 rounded-md border bg-background px-3 py-2 outline-none focus:border-primary focus:ring-2 focus:ring-ring/30"/><Button className="pop pop-ink rounded-xl" disabled={creating}>{creating ? 'Starting…' : 'Start exploring'}</Button></form>
