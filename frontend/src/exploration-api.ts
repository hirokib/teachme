...
export type ExplorationThread = { id: number; spaceId: number; parentThreadId: number | null; sourceMessageId: number | null; sourceExcerpt: string; title: string; intent: ThreadIntent; contextSummary: string; createdAt: string; updatedAt: string };
...
export type ThreadDetail = { space: ExplorationSpace; thread: ExplorationThread; parent: ExplorationThread | null; messages: ExplorationMessage[]; verifications: MessageVerification[] };
...
export async function deleteExploration(id: number) { const response = await fetch(`${API_URL}/api/explorations/${id}`, { method: 'DELETE' }); if (!response.ok) await json(response); }
